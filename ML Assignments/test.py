...
#     decision_tree = {best_feature:{}}
    ...
    if type_of_feature == "CONTINUOUS":
        ...
        feature_val_question_left_partition = "{} <= {}".format(names[best_feature], max_split_val_continuous)
        feature_val_question_right_partition = "{} > {}".format(names[best_feature], max_split_val_continuous)
        
        decision_tree[names_columns[best_feature]][feature_val_question_left_partition] = decision_tree_fit(names_columns,
        ...
        decision_tree[names_columns[best_feature]][feature_val_question_right_partition] = decision_tree_fit(names_columns,
        ...
        for feature_val in unique_feature_vals:
            ...
            if type_of_feature == "BINARIZED":
                if feature_val == 0:
                    feature_val_question = "{} <= {}".format(names_columns[best_feature], 
                                     dictionary_of_binarized_split_vals[best_feature][0])
                elif feature_val == 1:
                    feature_val_question = "{} > {}".format(names_columns[best_feature], 
                                     dictionary_of_binarized_split_vals[best_feature][0])
            else:  # CATEGORICAL
                feature_val_question = "{} == {}".format(names_columns[best_feature], feature_val)
            if len(partition_examples) == 1:
    #             decision_tree[best_feature][feature_val] = return_majority_label(labels)
                decision_tree[names_columns[best_feature]][feature_val_question] = return_majority_label(labels)
            else:
    #             decision_tree[best_feature][feature_val] = ...
                decision_tree[names_columns[best_feature]][feature_val_question] = decision_tree_fit(names_columns,
