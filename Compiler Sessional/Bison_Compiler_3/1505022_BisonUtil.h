...
public:
	...
	{
		char* errorFile = "1505022_BisonError.txt";
		char* logFile = "1505022_BisonLog.txt";
        errout.open(errorFile);
        logout.open(logFile);
        //debug.open("DEBUG_BISON.txt");
        arr.open("Array_Debug.txt");
        sem.open("1505022_Semantic.txt");
        err.open("1505022_ZeroError.txt");
	...
	void exitScope()
	{
		...
		//DEBUG
		string str = tab->printCurrentNonEmpty();
		printArrayDebug(str);
    ...
    void checkForDeclareAndPush(SymbolInfoNode *node)
    {
    	...
    	if(res){
    		if(res->isFunction == false){
    			...
    			printSemantic("Go for matchWithDeclareAndPush(node, res)");
    			matchWithDeclareAndPush(node, res);
    			return;
			} 
    	}else{
			printSemantic("Go for pushToSymbolTable( ... )");
			pushToSymbolTable(node->getName(), node->getType(), node->getDataType(), node->getLineBegin(),
				node->isFunction, node->isArray, node->functionItems, node->arrayItems, node->actualValue); 
	   	}
