...
public :
	...
	void assignOp(SymbolInfoNode *src, SymbolInfoNode *lhs, SymbolInfoNode *rhs, const BisonUtil &butil)
	{
		...
		for(int i=0; i<rhs->getName().length(); i++){
			if(rhs->getName()[i] == '['){
				...
				rhsName+= rhs->getName()[i] ;
			}
		}
		printf("\nBLA BLA BLA BLA BLA BLA BLA BLA BLA BLA BLA BLA \n\n\n");

		bool rhsChecker = isValidNameOfID(rhsName, butil);

		butil.printSemantic("==-->>>Inside assignOp, lhs name = " + lName + ", rhs.name = " + rhsName);
		SymbolInfoNode *resArray ;
		if(rhsChecker == true){
			resArray = butil.tab->LookUp(rhsName);
		}

		if(resArray){
			cout << "RESARRAY FOUND!! " << resArray->getPrintForTable() ;
		}else{
			printf("resArray not FOUND!!");
		}

		if(resArray){
			if(resArray->isArray == 1){
				butil.printSemantic("==-->>>Inside assignOp: resArray  = " + resArray->getPrintForTable());
				if(!isThisArray){
					assignRightArray(src, lhs, rhs, butil);
					return ;
				}
			}
		}*/
	...
	SymbolInfoNode *getNode(SymbolInfoNode * node, const BisonUtil &butil){
 	...
 	void addOp(SymbolInfoNode *source, SymbolInfoNode *lNode, SymbolInfoNode *rNode, SymbolInfoNode *operation, 
 		const BisonUtil &butil){


 		string strDeb = "==-->>> DEBUGGING .. Inside addOp, printing \nlNode = ";
 		strDeb += lNode->getPrintForTable();
 		strDeb += "\nrNode = ";
 		strDeb += rNode->getPrintForTable();
 		butil.printSemantic(strDeb);
 		...
 		if((lNode->dataType == "FLOAT")||(rNode->dataType == "FLOAT")){
 			finalDataType = "FLOAT";
 		}else{
 			finalDataType = "INT";
 		}
 		...
 		finalRes = converter.floatToString(res);
 		source->dataType = finalDataType;
 		source->actualValue = finalRes;
		string s = "=======>>>>> ====>>>> AFTER addOp of \n" + lNode->getPrintForTable() + " \n" + rNode->getPrintForTable();
		s += "\nSrc =" + source->getPrintForTable();
		butil.printSemantic(s);
 	}
...
Insert(string symbol_name, string symbol_type, string dataType, int lineBeg, bool isArray, bool isFunction, 
...
*/
 	...
 	void makeSymbol(SymbolInfoNode *src, SymbolInfoNode *n1, SymbolInfoNode *n2, SymbolInfoNode *n3, SymbolInfoNode *n4, 
 		...
 		butil.printSemantic("=============>>>> [1].After makeSymbol ... printing \nsrc=" + src->getPrintForTable());
 		butil.printSemantic("=============>>>> [2].After makeSymbol ... printing \n$1=" + n1->getPrintForTable());
 	}
